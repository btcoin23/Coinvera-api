...
import { getPumpTokenPrice } from './pumpfun';
import { getMoonshotTokenPrice } from './moonshot';
import { getRayAmmPrice, getRayClmmPrice, getRayCpmmPrice } from './raydium';
import { getMeteoraAmmTokenPrice, getMeteoraDlmmTokenPrice } from './meteora';
...
router.get('/price', async (req, res) => {
  ...
  try{
    ...
    if(ca){
      ...
      console.log(`\n-[GET] Batch Request: ${tokenAddresses.length} tokens, ${Date.now() - now_t}ms`);
...
async function getTokenInfo(ca: string){
